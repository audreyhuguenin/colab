...
const documentSlice = createSlice({
  ...
  initialState,
  reducers: {
    resetLastInsertedDocId: (state, action: PayloadAction<{ context: DocumentContext }>) => {
      if (action.payload.context.kind === 'DeliverableOfCardContent') {
        delete state.lastInsertedByCardContent[action.payload.context.ownerId];
      } else if (action.payload.context.kind === 'PartOfResource') {
        delete state.lastInsertedByResource[action.payload.context.ownerId];
      }
    },
  },
  extraReducers: builder =>
    builder
      ...
      .addCase(API.addDeliverable.fulfilled, (state, action) => {
      ...
      .addCase(API.addDocumentToResource.fulfilled, (state, action) => {
